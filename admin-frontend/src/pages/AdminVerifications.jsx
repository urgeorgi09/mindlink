...
const AdminVerifications = () => {
  ...
  const handleVerification = async (id, status, reason = null) => {
    try {
      ...
      toast.success(`Верификацията е ${status === 'approved' ? 'одобрена' : 'отхвърлена'}`);
      fetchVerifications();
    } catch (error) {
      toast.error('Грешка: ' + (error.response?.data?.message || 'Опитайте отново'));
    }
  };
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                ...
                <div style={{
                  ...
                  <span style={{ color: colors.warning, fontWeight: '600', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <ClockIcon style={{ width: "16px", height: "16px", strokeWidth: 2 }} />
                    Чака одобрение
                  </span>
                </div>
              </div>
