...
app.post('/api/admin/therapists/verify', async (req, res) => {
    try {
        ...
        try {
            const mainBackendUrl = process.env.MAIN_BACKEND_URL || 'http://backend:5000';
            await axios.post(`${mainBackendUrl}/api/admin/therapists/verify`, { therapistId });
        } catch (syncError) {
            console.error('Failed to sync to main database:', syncError.message);
        }
...
// Sync data from main backend (mindlink database)
app.get('/api/sync/main-data', async (req, res) => {
    try {
        const mainBackendUrl = process.env.MAIN_BACKEND_URL || 'http://backend:5000';
        const response = await axios.get(`${mainBackendUrl}/api/admin/sync-data`);
        const data = response.data;
        
        // Insert/update users
        for (const user of data.users) {
            await pool.query(
                `INSERT INTO users (id, name, email, password, role, specialty, verified, uin, created_at) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) 
                 ON CONFLICT (email) DO UPDATE SET 
                 name = $2, role = $5, specialty = $6, verified = $7, uin = $8`,
                [user.id, user.name, user.email, 'synced', user.role, user.specialty, user.verified, user.uin, user.created_at]
            );
        }
        ...
        for (const msg of data.messages) {
            await pool.query(
                `INSERT INTO messages (id, text, sender_id, recipient_id, timestamp, is_read, is_important) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (id) DO NOTHING`,
                [msg.id, msg.text, msg.sender_id, msg.recipient_id, msg.timestamp, msg.is_read, msg.is_important]
            );
        }
        ...
        for (const mood of data.moodEntries) {
            await pool.query(
                `INSERT INTO mood_entries (id, user_id, mood, energy, anxiety, notes, date) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (id) DO NOTHING`,
                [mood.id, mood.user_id, mood.mood, mood.energy, mood.anxiety, mood.notes, mood.date]
            );
        }
        ...
        for (const journal of data.journalEntries) {
            await pool.query(
                `INSERT INTO journal_entries (id, user_id, title, content, category, word_count, date) 
                 VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (id) DO NOTHING`,
                [journal.id, journal.user_id, journal.title, journal.content, journal.category, journal.word_count, journal.date]
            );
        }
        ...
        for (const note of data.notes) {
            await pool.query(
                `INSERT INTO notes (id, title, content, patient_id, therapist_id, date) 
                 VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO NOTHING`,
                [note.id, note.title, note.content, note.patient_id, note.therapist_id, note.date]
            );
        }
        
        // Insert therapist-patient relationships
        for (const rel of data.therapistPatients) {
            await pool.query(
                `INSERT INTO therapist_patients (id, therapist_id, patient_id, assigned_at) 
                 VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING`,
                [rel.id, rel.therapist_id, rel.patient_id, rel.assigned_at]
            );
        }
        
        res.json({ message: 'Синхронизацията е успешна', synced: data });
    } catch (error) {
